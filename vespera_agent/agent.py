...
# ========= Custom =========
from vespera_agent.prompt_hub import BkokPromptHub
from vespera_utils import complete_explorer_aptos_url
from vespera_agent.math_stats import step_1_stat_txs, step_2_agent_pre_analysis
from vespera_agent.nillion_tools import NillionManager, create_nillion_manager
...
def create_bkok_agent_wrapper(explorer_url_prefix: str, 
                               agent_name: str, 
    ...
    if explorer_api_key is None:
        raise ValueError("explorer_API_KEY environment variable is not set.")
    ...
    def fetch_tx_agent_func(context_variables: dict, wallet_address: str) -> str:
        ...
        explorer_txlist_url = f"http://18.143.117.194:8080/transactions?target={wallet_address}"
        response = requests.get(explorer_txlist_url)
